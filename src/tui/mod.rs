//! Terminal UI — Elm/TEA architecture.
//!
//! Model:  `state::AppState`
//! Message: `message::Message` (from `event`)
//! Update:  key/mouse handlers in `update`
//! View:    `view::draw_ui`
...
use crate::db::{DbRequest, DbResponse, DbWorker};
use crossterm::{
    event::{
...
use state::{
    ActiveEngine, ActivePane, AppState, ConnectionMode, ExplorationState, FormField, ToolbarAction,
};
...
fn load_related_data_if_needed(state: &AppState) -> Option<DbRequest> {
    ...
    let col_pos = state
        .result_headers
        .iter()
        .position(|c| c.to_lowercase() == rel.active_col.to_lowercase())?;
...
pub fn extract_active_table(query: &str, engine: ActiveEngine) -> Option<String> {
    match engine {
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while !state.should_quit {
                ...
                DbResponse::Tables(tbls) => {
                    ...
                    if !tbls.is_empty() {
                        ...
                        let query = match state.active_engine {
                            ...
                            ActiveEngine::MongoDb => Some(format!("{}|{{}}", table)),
                            ActiveEngine::Neo4j => {
                        ...
                        match &res {
                            DbResponse::Error(err) => {
                                state.conn_status_msg =
                                    format!("Error executing change: {}", err);
        ...
        if poll(Duration::from_millis(50))? {
            match read()? {
                // Windows sends Press *and* Release for every key; only Press is meaningful here.
                Event::Key(key) if key.kind == KeyEventKind::Press => {
                    ...
                    if state.connected
                        ...
                        && state.active_pane != ActivePane::ModalEditor
                        && !state.show_delete_confirm
                        && !state.search_active
                        && !state.show_describe
                        && !state.show_trace
                        && !state.show_row_detail
                        && !state.sidebar_filter_active
                    {
                        match key.code {
                            ...
                            KeyCode::Char('q') | KeyCode::Esc => {
                                if state.active_pane == ActivePane::EngineSelector
                                    && state.connection_mode == ConnectionMode::Profiles
                                {
                                    state.should_quit = true;
                                    break;
                                }
                    ...
                    match state.active_pane {
                        ActivePane::EngineSelector => match key.code {
                            ...
                            KeyCode::Up => match state.connection_mode {
                                ConnectionMode::Profiles => {
                                    ...
                                    if !matching.is_empty() {
                                        let pos = matching
                                            .iter()
                                            .position(|&x| x == state.selected_profile_idx)
                                            .unwrap_or(0);
                                        if pos > 0 {
                            ...
                            KeyCode::Down => match state.connection_mode {
                                ConnectionMode::Profiles => {
                                    let matching: Vec<usize> = state
                                        .profiles
                                        .iter()
                            ...
                            KeyCode::Char(c) => match state.connection_mode {
                                ConnectionMode::Profiles => {}
                                ConnectionMode::Form => match state.active_form_field {
                                ...
                                ConnectionMode::RawUrl => match state.active_engine {
                                    ActiveEngine::MariaDb => state.conn_fields.mysql_url.push(c),
                                    ActiveEngine::PostgreSql => {
                                        state.conn_fields.postgres_url.push(c)
                                    }
                            ...
                            KeyCode::Backspace => match state.connection_mode {
                                ConnectionMode::Profiles => {}
                                ConnectionMode::Form => match state.active_form_field {
                                    FormField::Host => {
                                        state.form_fields.host.pop();
                                    }
                                    FormField::Port => {
                                        state.form_fields.port.pop();
                                    }
                                    FormField::User => {
                                        state.form_fields.user.pop();
                                    }
                                    FormField::Pass => {
                                        state.form_fields.pass.pop();
                                    }
                                    FormField::Db => {
                                        state.form_fields.db_or_path.pop();
                                    }
                                },
                                ConnectionMode::RawUrl => match state.active_engine {
                                    ActiveEngine::MariaDb => {
                                        state.conn_fields.mysql_url.pop();
                                    }
                                    ActiveEngine::PostgreSql => {
                                        state.conn_fields.postgres_url.pop();
                                    }
                                    ActiveEngine::Sqlite => {
                                        state.conn_fields.sqlite_path.pop();
                                    }
                                    ActiveEngine::MongoDb => {
                                        state.conn_fields.mongodb_url.pop();
                                    }
                                    ActiveEngine::Neo4j => {
                                        state.conn_fields.neo4j_url.pop();
                                    }
                                    ActiveEngine::LocalJson => {
                                        state.conn_fields.json_path.pop();
                                    }
                                },
                            },
                            _ => {}
                                    ...
                                    if state.show_db_list {
                                        if let Some(idx) = state.selected_db_idx {
                                            if idx < state.databases.len() {
                                                ...
                                                state.connecting = true;
                                                state.conn_status_msg =
                                                    format!("Switching database to {}...", db_name);
                                                state.show_db_list = false;
                                                let _ = app_tx
                                                    .send(DbRequest::SelectDatabase(db_name))
                                                    .await;
                                            }
                                        }
                                    } else {
                                        ...
                                        if let Some(idx) = state.selected_table_idx {
                                            ...
                                            state.connecting = true;
                                            let _ =
                                                app_tx.send(DbRequest::ExecuteQuery(query)).await;
                                            let _ = app_tx
                                                .send(DbRequest::LoadMetadata { table })
                                                .await;
                                        }
                                    }
                                }
                        ...
                        ActivePane::SqlConsole => match key.code {
                            KeyCode::Char(c) => {
                            ...
                            KeyCode::Enter => {
                                ...
                                if !state.sql_console_input.trim().is_empty() {
                                    ...
                                    if let Some(table) =
                                        extract_active_table(&query, state.active_engine)
                                    {
                                        state.active_table_name = Some(table.clone());
                                        let _ =
                                            app_tx.send(DbRequest::LoadMetadata { table }).await;
                                    } else {
                        ...
                        ActivePane::QueryResults => {
                                    ...
                                    KeyCode::Enter => {
                                        if let Some(query) = state.compile_delete_statement() {
                                            if let Some(ref tbl) = state.active_table_name {
                                                let reload =
                                                    format!("SELECT * FROM {} LIMIT 50;", tbl);
                                                state.mutation_in_progress = Some(reload);
                                                state.connecting = true;
                                                let _ = app_tx
                                                    .send(DbRequest::ExecuteQuery(query))
                                                    .await;
                                            }
                                        }
                                    ...
                                    KeyCode::Char(c) => {
                                        if state.pivot_state.active_selector_idx == 6 {
                                            ...
                                            if state.pivot_state.config.auto_recalc {
                                                state.pivot_state.config.filter_val =
                                                    state.pivot_state.filter_text_input.clone();
                                                state.recalculate_pivot();
                                            }
                                        }
                                    }
                            ...
                            if state.show_row_detail {
                                match key.code {
                                    ...
                                    KeyCode::Up => {
                                        state.row_detail_scroll =
                                            state.row_detail_scroll.saturating_sub(1);
                                    }
                                    KeyCode::Down => {
                                    ...
                                    KeyCode::PageUp => {
                                        state.row_detail_scroll =
                                            state.row_detail_scroll.saturating_sub(10);
                                    }
                                    KeyCode::PageDown => {
                            ...
                            if state.show_trace {
                                match key.code {
                                    ...
                                    KeyCode::Char('j') | KeyCode::Char('J') => {
                                        ...
                                        state.trace_scroll_desc = 0;
                                    }
                                    KeyCode::Tab | KeyCode::BackTab => {
                                        if !state.trace_json_mode {
                                            state.trace_focus_ancestors =
                                                !state.trace_focus_ancestors;
                                        }
                                    }
                                    KeyCode::Up => {
                                        if state.trace_focus_ancestors || state.trace_json_mode {
                                            state.trace_scroll =
                                                state.trace_scroll.saturating_sub(1);
                                        } else {
                                            state.trace_scroll_desc =
                                                state.trace_scroll_desc.saturating_sub(1);
                                        }
                                    }
                                    KeyCode::Down => {
                                        if state.trace_focus_ancestors || state.trace_json_mode {
                                            ...
                                            state.trace_scroll_desc = state.trace_scroll_desc + 1;
                                        }
                                    }
                                    KeyCode::PageUp => {
                                        if state.trace_focus_ancestors || state.trace_json_mode {
                                            state.trace_scroll =
                                                state.trace_scroll.saturating_sub(10);
                                        } else {
                                            state.trace_scroll_desc =
                                                state.trace_scroll_desc.saturating_sub(10);
                                        }
                                    }
                                    KeyCode::PageDown => {
                                        if state.trace_focus_ancestors || state.trace_json_mode {
                                            ...
                                            state.trace_scroll_desc = state.trace_scroll_desc + 10;
                                        }
                                    }
                                    KeyCode::Home => {
                                        if state.trace_focus_ancestors || state.trace_json_mode {
                            ...
                            let is_dbf = state.active_engine == ActiveEngine::LocalJson
                                && state.conn_fields.json_path.ends_with(".dbf");
                            let is_document_view = (state.active_engine == ActiveEngine::MongoDb
                                || state.active_engine == ActiveEngine::LocalJson)
                                && !is_dbf;
                            match key.code {
                                KeyCode::Char('/') => {
                                    if !is_document_view {
                                ...
                                KeyCode::Char('r') | KeyCode::Char('R') => {
                                    if let Some(req) =
                                        state.trigger_toolbar_action(ToolbarAction::Refresh)
                                    {
                                        let _ = app_tx.send(req).await;
                                        if let Some(ref table) = state.active_table_name {
                                            let _ = app_tx
                                                .send(DbRequest::LoadMetadata {
                                                    table: table.clone(),
                                                })
                                                .await;
                                        }
                                ...
                                KeyCode::Enter | KeyCode::Char(' ') => {
                                    ...
                                    if (state.active_engine == ActiveEngine::MongoDb
                                        || state.active_engine == ActiveEngine::LocalJson)
                                        && !is_dbf
                                    {
                                    ...
                                    if let Some(idx) = state.selected_row_idx {
                                        if idx < state.result_rows.len() {
                                            ...
                                            if let Some(ref pk_col) = state.primary_key {
                                                if let Some(pos) =
                                                    state.result_headers.iter().position(|x| {
                                                        x.to_lowercase() == pk_col.to_lowercase()
                                                    })
                                                {
                                                    state.selected_row_pk_val =
                                                        state.result_rows[idx].get(pos).cloned();
                                                }
                                            }
                                        }
                                    }
                                ...
                                KeyCode::End => {
                                    state.selected_col_idx =
                                        state.result_headers.len().saturating_sub(1);
                                }
                                KeyCode::Esc => {
                                    state.active_pane = ActivePane::SqlConsole;
                                }
                                KeyCode::Backspace | KeyCode::Char('b') | KeyCode::Char('B') => {
                                    if !state.exploration_history.is_empty() {
                                        if let Some(prev) = state.exploration_history.pop() {
                                            ...
                                            state.active_relationship_idx =
                                                prev.active_relationship_idx;
                                            state.show_related_split = prev.show_related_split;
                                            state.connecting = true;
                                            let _ = app_tx
                                                .send(DbRequest::ExecuteQuery(prev.query))
                                                .await;
                        ...
                        ActivePane::RelatedDataList => match key.code {
                            ...
                            KeyCode::Down | KeyCode::Char('s') | KeyCode::PageDown => {
                                if !state.relationships.is_empty() {
                                    if state.active_relationship_idx
                                        < state.relationships.len() - 1
                                    {
                                        state.active_relationship_idx += 1;
                                    } else {
                        ...
                        ActivePane::RelatedDataGrid => match key.code {
                            KeyCode::Left => {
                            ...
                            KeyCode::Up | KeyCode::Char('w') | KeyCode::PageUp => {
                                if !state.related_rows.is_empty() {
                                    ...
                                    if current > 0 {
                                        ...
                                        state.related_selected_row_idx =
                                            Some(state.related_rows.len() - 1);
                                    }
                                }
                            }
                            KeyCode::Down | KeyCode::Char('s') | KeyCode::PageDown => {
                            ...
                            KeyCode::Enter | KeyCode::Char('g') | KeyCode::Char('G') => {
                                if state.related_selected_row_idx.is_some() {
                                    if state.active_relationship_idx < state.relationships.len() {
                                        let rel =
                                            state.relationships[state.active_relationship_idx]
                                                .clone();
                                        ...
                                        if let Some(row_idx) = state.selected_row_idx {
                                            if let Some(row) = state.result_rows.get(row_idx) {
                                                if let Some(col_pos) =
                                                    state.result_headers.iter().position(|c| {
                                                        c.to_lowercase()
                                                            == rel.active_col.to_lowercase()
                                                    })
                                                {
                                                    if let Some(active_val) = row.get(col_pos) {
                                                        ...
                                                        let current_state = ExplorationState {
                                                            ...
                                                            query: state
                                                                .sql_console_input
                                                                .clone(),
                                                            selected_row_idx: state
                                                                .selected_row_idx,
                                                            active_relationship_idx: state
                                                                .active_relationship_idx,
                                                            show_related_split: state
                                                                .show_related_split,
                                                        };
                        ...
                        ActivePane::ModalEditor => match key.code {
                            KeyCode::Esc => {
                            ...
                            KeyCode::Up => {
                                if !state.modal_fields.is_empty() {
                                    if state.active_modal_field_idx > 0 {
                                        ...
                                        state.active_modal_field_idx =
                                            state.modal_fields.len() - 1;
                                    }
                                }
                            }
                            KeyCode::Down => {
                ...
                Event::Mouse(mouse_event) => {
                    ...
                    if matches!(
                        ...
                        match state.active_pane {
                            ActivePane::Sidebar => {
                                let len = if state.show_db_list {
                                    state.databases.len()
                                } else {
                                    state.tables.len()
                                };
                                let cur = if state.show_db_list {
                                    &mut state.selected_db_idx
                                } else {
                                    &mut state.selected_table_idx
                                };
                                if let Some(idx) = cur {
                                    if forward {
                                        if *idx + 1 < len {
                            ...
                            ActivePane::QueryResults => {
                                if is_document_view {
                                    if let Some(idx) = state.selected_tree_row_idx {
                                        if forward {
                                            if idx + 1 < state.flat_tree_rows.len() {
                                                state.selected_tree_row_idx = Some(idx + 1);
                                            }
                                        } else if idx > 0 {
                                            state.selected_tree_row_idx = Some(idx - 1);
                            ...
                            ActivePane::RelatedDataGrid => {
                                if !state.related_rows.is_empty() {
                                    ...
                                    if forward {
                                        if cur + 1 < state.related_rows.len() {
                                            state.related_selected_row_idx = Some(cur + 1);
                                        }
                                    } else if cur > 0 {
                                        state.related_selected_row_idx = Some(cur - 1);
                                    }
                    ...
                    if mouse_event.kind == MouseEventKind::Down(MouseButton::Left) {
                        ...
                        } else if let Some(req) = load_related_data_if_needed(&state) {
                            let _ = app_tx.send(req).await;
                        }
                    }
                }
